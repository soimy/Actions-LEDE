...
- `configs/packages-x86.txt` 开启 `CONFIG_TOOLCHAINOPTS=y` 后使用当前 LEDE master 的 `CONFIG_BINUTILS_USE_VERSION_2_43=y`，并在编译前校验解析结果为 `2.43.1`；
...
当前 LEDE master 已从旧的 `CONFIG_BINUTILS_VERSION_2_43_1` 选择符切换为 `CONFIG_BINUTILS_USE_VERSION_2_43`。由于该 choice 受 `TOOLCHAINOPTS` 条件控制，必须同时设置 `CONFIG_TOOLCHAINOPTS=y`；否则 `make defconfig` 会静默回退到默认 binutils 2.42。工作流现在会在真正编译前直接报出解析版本，避免浪费几十分钟后才失败。
